...
namespace OZI_lab2_infocollector
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                computerInfo += monitor["CurrentHorizontalResolution"].ToString();
                Console.WriteLine("Monitor Width : {0}", monitor["CurrentHorizontalResolution"].ToString());
            }
        ...
        public static void SaveEncryptedInfo(string encryptedInfo) 
        {
            ...
            RegistryKey personalKey = userKey.CreateSubKey("Ivanchenko_");
            personalKey.SetValue("Ivanchenko", encryptedInfo);
            personalKey.Close();
            Console.WriteLine();
        ...
        public static void ReadEncryptedInfo() 
        {
            ...
            RegistryKey personalKey = currentUserKey.OpenSubKey("Ivanchenko_", true);

            try
            {
                string signedData = personalKey.GetValue("Ivanchenko).ToString();
        ...
        {
            RegistryKey currentUserKey = Registry.CurrentUser;
            RegistryKey helloKey = currentUserKey.OpenSubKey("Ivanchenko_", true);
            helloKey.DeleteValue("Ivanchenko");
            currentUserKey.DeleteSubKey("Ivanchenko_");
        }
    }
}
